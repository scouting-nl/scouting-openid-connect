...
class LoggingDownload
{
    ...
    public function download(Logging $logging, array $filters): void {
        ...
        fwrite($output, "Format: [created_at] [level] [component] [user_id] [sol_id] message\n\n");

        // Fetch and output logs
        $rows = $logging->get_logs($filters, $sorting);
        $padding = $this->determine_padding($rows);
        if (empty($rows)) {
            fwrite($output, "# No logs found for the applied filters.\n");
        } else  {
            foreach ($rows as $row) {
                fwrite($output, $this->format_log_line($row, $padding));
            }
        }
