...
class User {
	...
	public function scouting_oidc_user_create(): void {
		...
		if ( is_wp_error( $user_id ) && $user_id->get_error_code() === 'existing_user_email' ) {
			Logger::warning( LogComponent::USER, "Creating user '{$this->fullName}' failed due to email conflict for '{$this->email}'", null, $this->sol_id );
			if ( get_option( 'scouting_oidc_user_duplicate_email', 'plus_addressing' ) === 'plus_addressing' ) {
				Logger::info( LogComponent::USER, "Creating user '{$this->fullName}' with plus-addressing strategy to resolve email conflict", null, $this->sol_id );
				...
				if ( $user_id_by_email && $user_id_by_email !== username_exists( $this->sol_id ) ) {
					Logger::error( LogComponent::USER, "Creating user '{$this->fullName}' failed: plus-addressed email '{$plusAddressEmail}' is already linked to another account", null, $this->sol_id );
		...
		Logger::info( LogComponent::USER, "User '{$this->fullName}' created successfully", $user_id, $this->sol_id );
	...
	public function scouting_oidc_user_update(): void {
		...
		if ( $user_id_by_sol_id && $user_id_by_email && $user_id_by_sol_id === $user_id_by_email ) {
			Logger::info( LogComponent::USER, "Updating user '{$this->fullName}' where SOL ID and email both match the same existing account", $user_id_by_sol_id, $this->sol_id );
			// Update meta data
		...
		elseif ( $user_id_by_sol_id && $user_id_by_email && $user_id_by_sol_id !== $user_id_by_email ) {
			...
			if ( get_option( 'scouting_oidc_user_duplicate_email' ) === 'plus_addressing' ) {
				Logger::info( LogComponent::USER, "Updating user '{$this->fullName}' email address using plus-addressing strategy to resolve conflict", $user_id_by_sol_id, $this->sol_id );
				...
				if ( $user_id_by_plus_address_email && $user_id_by_plus_address_email !== $user_id_by_sol_id ) {
					Logger::error( LogComponent::USER, "Updating user '{$this->fullName}' failed: plus-addressed email '{$plusAddressEmail}' is already linked to another account", $user_id_by_sol_id, $this->sol_id );
					ErrorHandler::redirect_to_login_error( 'error', __( 'Email address is already in use by another account', 'scouting-openid-connect' ), 'login_email_mismatch' );
				}
				...
				$result = wp_update_user(
					array(
						...
						'user_email' => $plusAddressEmail,
					)
				);
				if ( is_wp_error( $result ) ) {
					Logger::log_wp_error( LogComponent::USER, LogLevel::ERROR, $result, $user_id_by_sol_id, $this->sol_id );
				} else {
					Logger::info( LogComponent::USER, "Updating user '{$this->fullName}' email address to plus-addressed version '{$plusAddressEmail}' succeeded", $user_id_by_sol_id, $this->sol_id );
				}
			} else {
				Logger::error( LogComponent::USER, "Updating user '{$this->fullName}' failed: Email conflict for '{$this->email}' and duplicate-email strategy is not plus-addressing", $user_id_by_sol_id, $this->sol_id );
				ErrorHandler::redirect_to_login_error( 'error', __( 'Email address is already in use by another account', 'scouting-openid-connect' ), 'login_email_mismatch' );
			}
		...
		elseif ( $user_id_by_sol_id && ! $user_id_by_email ) {
			...
			if ( is_wp_error( $result ) ) {
				...
				Logger::info( LogComponent::USER, "Updating user '{$this->fullName}' email address succeeded", $user_id_by_sol_id, $this->sol_id );
			}
		...
		// User not found by either SOL ID or email
			...
			Logger::error( LogComponent::USER, "Updating user '{$this->fullName}' failed: no user found for SOL ID '{$this->sol_id}' or email '{$this->email}'", null, $this->sol_id );
		...
		Logger::info( LogComponent::USER, "Updating user '{$this->fullName}' finished", $user_id_by_sol_id, $this->sol_id );
	}
	...
	public function scouting_oidc_user_login(): void {
		...
		if ( ! $user ) {
			Logger::error( LogComponent::USER, "User '{$this->fullName}' failed to log in: no user found for SOL ID '{$this->sol_id}'", null, $this->sol_id );
		...
		Logger::info( LogComponent::USER, "User '{$this->fullName}' logged in successfully", $user->ID, $this->sol_id );
	...
	private function scouting_oidc_user_update_meta( int $user_id ): void {
		Logger::info( LogComponent::USER, "Updating user '{$this->fullName}' meta data", $user_id, $this->sol_id );
		...
		Logger::info( LogComponent::USER, "Updating user '{$this->fullName}' meta data finished", $user_id, $this->sol_id );
	}
	...
	private function scouting_oidc_user_sync_to_woocommerce( int $user_id ): void {
		Logger::info( LogComponent::USER, "Syncing user '{$this->fullName}' data to WooCommerce", $user_id, $this->sol_id );
		...
		Logger::info( LogComponent::USER, "Syncing user '{$this->fullName}' data to WooCommerce finished", $user_id, $this->sol_id );
	}
}
