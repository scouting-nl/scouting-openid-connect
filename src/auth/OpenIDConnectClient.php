...
class OpenIDConnectClient {
	 ...
	 */
	private static array $logoutRedirectHosts = array();
	...
	public function retrieve_tokens( string $code, ?string $state = null ): void {
		...
		$code_verifier = ( $state !== null ) ? $this->get_code_verifier_for_state( $state ) : null;
	 ...
	 *
	 * @param string $stored_nonce The nonce saved before retrieve_tokens cleared session state.
	...
	public function validate_tokens( string $stored_nonce = '' ): array {
		...
		$header    = json_decode( $this->base64_url_decode( $headerEncoded ), true );
		$payload   = json_decode( $this->base64_url_decode( $payloadEncoded ), true );
		$signature = $this->base64_url_decode( $signatureEncoded );
